...
    // Show/hide the binary conversion button depending on the loaded example.
    ...
    if (binaryConversionBtn) {
        if (doc.isExample && doc.id !== 'add-binary-3-tape') {
...
// =========================================================================
// ▼▼▼ NEW BINARY CONVERSION FUNCTION ADDED HERE ▼▼▼
// =========================================================================
function handleBinaryConversion() {
  ...
  // The controller holds the simulator, which has the machine and its rules.
  ...
  for (const fromState in machineRules) {
    for (const readSymbol in machineRules[fromState]) {
      ...
      const newStateCode = binaryMap.states[rule.state]; // In your structure, it's 'state'
      const writeSymbolCode = binaryMap.symbols[rule.write]; // In your structure, it's 'write'
      const moveDirectionCode = binaryMap.directions[rule.move]; // In your structure, it's 'move'

      // Check if all parts were found in the map
